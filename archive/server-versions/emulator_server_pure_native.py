#!/usr/bin/env python3
"""
ZX Spectrum Emulator Server - Pure Native Resolution
====================================================

This version shows the pure native ZX Spectrum resolution (320x240)
centered in the HD frame with NO scaling whatsoever.

CONFIGURATION:
- Native capture: 320x240
- NO scaling applied at all
- Direct pad to center in 1280x720 frame
- Cursor hidden
- Synthetic audio
"""
...
class PureNativeEmulatorServer:
    def __init__(self):
        ...
        self.version = os.getenv('VERSION', '1.0.0-pure-native')
        self.build_time = os.getenv('BUILD_TIME', '2025-08-04T01:35:00Z')
        
        # PURE NATIVE: No scaling configuration
        ...
        logger.info("🎮 Starting Pure Native ZX Spectrum Emulator Server")
        logger.info(f"Version: {self.version}")
        logger.info(f"Strategy: Pure native 320x240, NO scaling, centered in HD")
        logger.info("")
        logger.info("🎯 Pure Native Configuration:")
        logger.info(f"  Virtual Display: {self.virtual_display_size}")
        logger.info(f"  Capture Size: {self.capture_size} (PURE NATIVE)")
        ...
        logger.info(f"  Scaling: ABSOLUTELY NONE - Pure 320x240")
    ...
    def start_ffmpeg_hls(self):
        """Start FFmpeg HLS streaming with PURE NATIVE resolution"""
        try:
            logger.info("Starting FFmpeg HLS with PURE NATIVE resolution...")
            logger.info(f"Pure native: {self.capture_size} → directly centered in {self.output_resolution}")
            
            # PURE NATIVE: Just pad the native resolution to center in HD frame
            ffmpeg_cmd = [
                'ffmpeg',
                # Input: X11 grab
                '-f', 'x11grab',
                ...
                # PURE NATIVE: Just pad to center - NO SCALING AT ALL
                '-vf', f'pad={self.output_resolution}:(ow-iw)/2:(oh-ih)/2:black,drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text=\'🔴 LIVE ZX SPECTRUM PURE NATIVE %{{localtime}}\':fontcolor=yellow:fontsize=20:x=w-text_w-20:y=20:box=1:boxcolor=black@0.7:boxborderw=3',
            ...
            logger.info(f"PURE NATIVE FFmpeg HLS command: {' '.join(ffmpeg_cmd)}")
            ...
            time.sleep(3)
            logger.info("✅ FFmpeg HLS started successfully with PURE NATIVE resolution")
    ...
    def start_ffmpeg_youtube(self):
        """Start FFmpeg YouTube RTMP streaming with PURE NATIVE resolution"""
        if not self.youtube_key:
            logger.info("No YouTube stream key provided, skipping RTMP stream")
            return
            
        try:
            logger.info("Starting YouTube RTMP stream with PURE NATIVE resolution...")
            
            # PURE NATIVE: YouTube command with no scaling
            youtube_cmd = [
                'ffmpeg', '-y',
            ...
            logger.info(f"PURE NATIVE YouTube FFmpeg command: {' '.join(youtube_cmd)}")
            ...
            time.sleep(3)
            logger.info("✅ YouTube RTMP streaming started successfully with PURE NATIVE resolution")
    ...
    async def handle_websocket(self, websocket):
        ...
        try:
            await websocket.send(json.dumps({
                ...
                'strategy': 'pure-native',
                'resolution': 'pure-320x240'
            }))
    ...
    async def start_server(self):
        """Start the pure native server"""
        try:
            logger.info("🚀 Starting pure native server components...")
            ...
            logger.info("🎮 Pure Native ZX Spectrum Emulator Server is fully operational!")
            logger.info("📺 Pure native 320x240 resolution, NO scaling, centered in HD frame")
...
def main():
    """Main entry point"""
    server = PureNativeEmulatorServer()
