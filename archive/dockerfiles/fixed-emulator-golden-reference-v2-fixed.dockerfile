...
# Copy the FIXED golden reference server code
COPY server/emulator_server_golden_reference_v2_fixed.py /app/emulator_server.py
...
# Version information
ENV VERSION=1.0.0-golden-reference-v2-fixed
ENV BUILD_TIME=2025-08-04T01:00:00Z

# Create startup script with user context fixes
RUN echo '#!/bin/bash\n\
echo "🏆 Starting Golden Reference ZX Spectrum Emulator Server v2 FIXED"\n\
echo "================================================================"\n\
echo "Version: $VERSION"\n\
echo "Build Time: $BUILD_TIME"\n\
echo "User: $(whoami)"\n\
echo "Home: $HOME"\n\
echo "Strategy: Fixed user context + proven local FUSE streaming + FIXED FFmpeg"\n\
echo ""\n\
echo "🎯 Configuration:"\n\
...
echo "  ✅ FFmpeg Filters: Fixed video filter chain syntax"\n\
...
# Start the FIXED golden reference server as spectrum user
CMD ["/app/start.sh"]
