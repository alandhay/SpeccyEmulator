#!/bin/bash

echo "Creating HIGH BITRATE YouTube streaming task definition..."

# Create task definition with enhanced video quality
cat > high-bitrate-task-definition.json << 'EOF'
{
  "family": "spectrum-emulator-streaming",
  "networkMode": "awsvpc",
  "requiresCompatibilities": ["FARGATE"],
  "cpu": "2048",
  "memory": "4096",
  "executionRoleArn": "arn:aws:iam::043309319786:role/ecsTaskExecutionRole",
  "taskRoleArn": "arn:aws:iam::043309319786:role/ecsTaskExecutionRole",
  "containerDefinitions": [
    {
      "name": "spectrum-emulator-streamer",
      "image": "043309319786.dkr.ecr.us-east-1.amazonaws.com/spectrum-emulator:high-bitrate",
      "essential": true,
      "portMappings": [
        {
          "containerPort": 8080,
          "hostPort": 8080,
          "protocol": "tcp"
        },
        {
          "containerPort": 8765,
          "hostPort": 8765,
          "protocol": "tcp"
        }
      ],
      "environment": [
        {
          "name": "DISPLAY",
          "value": ":99"
        },
        {
          "name": "SDL_VIDEODRIVER",
          "value": "x11"
        },
        {
          "name": "SDL_AUDIODRIVER",
          "value": "pulse"
        },
        {
          "name": "PULSE_RUNTIME_PATH",
          "value": "/tmp/pulse"
        },
        {
          "name": "STREAM_BUCKET",
          "value": "spectrum-emulator-stream-dev-043309319786"
        },
        {
          "name": "YOUTUBE_STREAM_KEY",
          "value": "0ebh-efdh-9qtq-2eq3-e6hz"
        },
        {
          "name": "CAPTURE_SIZE",
          "value": "512x384"
        },
        {
          "name": "DISPLAY_SIZE",
          "value": "1024x768"
        },
        {
          "name": "CAPTURE_OFFSET",
          "value": "0,0"
        },
        {
          "name": "VIDEO_BITRATE",
          "value": "5000k"
        },
        {
          "name": "YOUTUBE_BITRATE",
          "value": "6000k"
        },
        {
          "name": "OUTPUT_RESOLUTION",
          "value": "1920x1080"
        }
      ],
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/spectrum-emulator-streaming",
          "awslogs-region": "us-east-1",
          "awslogs-stream-prefix": "ecs"
        }
      },
      "healthCheck": {
        "command": [
          "CMD-SHELL",
          "curl -f http://localhost:8080/health || exit 1"
        ],
        "interval": 30,
        "timeout": 10,
        "retries": 3,
        "startPeriod": 180
      }
    }
  ]
}
EOF

echo "Registering high bitrate task definition..."
TASK_DEF_ARN=$(aws ecs register-task-definition \
  --cli-input-json file://high-bitrate-task-definition.json \
  --region us-east-1 \
  --query 'taskDefinition.taskDefinitionArn' \
  --output text)

echo "Task definition registered: $TASK_DEF_ARN"

# Update the service
echo "Updating service with high bitrate configuration..."
aws ecs update-service \
  --cluster spectrum-emulator-cluster-dev \
  --service spectrum-youtube-streaming \
  --task-definition "$TASK_DEF_ARN" \
  --region us-east-1

echo "Service update initiated. Monitoring deployment..."

# Wait for deployment to complete
aws ecs wait services-stable \
  --cluster spectrum-emulator-cluster-dev \
  --services spectrum-youtube-streaming \
  --region us-east-1

echo "HIGH BITRATE streaming deployment completed!"
echo "YouTube stream should now have much better quality at 1080p with 6Mbps bitrate"
echo ""
echo "Monitor the deployment:"
echo "aws ecs describe-services --cluster spectrum-emulator-cluster-dev --services spectrum-youtube-streaming --region us-east-1"
echo ""
echo "Check logs:"
echo "aws logs tail \"/ecs/spectrum-emulator-streaming\" --follow --region us-east-1"
